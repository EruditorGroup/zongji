...
ComBinlogGTID.prototype.write = function(writer) {
  ...
  let gtidsDataBytesLength = 0;
  for (let i = 0; i < gtidsDataEntries.length; i++) {
    const [sid, intervals] = gtidsDataEntries[i];
    // sid(16-byte) + intervals.length(int64) + intervals
    gtidsDataBytesLength += 16 + 8 + intervals.length * 16;
  }
  writer.writeUnsignedNumber(4, 8 + gtidsDataBytesLength); //data_length
